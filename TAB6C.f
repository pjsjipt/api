      SUBROUTINE TAB6C(ALF60,DEGF,VCFC,VCFP,IFLAG)
      
      double precision ALF60, DEGF, VCFC, VCFP
      integer IHYDRO, IFLAG


      DATA IEP1,IEP2,IEP3/2500,2000,1500/
      DATA NBP1,NBP2/2700,9300/
      DATA IBP1,IBP2/5100,5300/
      DATA ITMP1,ITMP2,ITMP3/3000,2500,2000/
      DATA IBAS /600/

      VCFC=-1.0000
      VCFP=-1.0000
      IFLAG=-1
      
      IALF=(ALF60*100000000.0+25.0)/50.0
      IALF=IALF*5
      ALF60=FLOAT(IALF)/10000000.
      
      ITEMP=DEGF*10.0 + 0.5
      DEGF=FLOAT(ITEMP)/10.0
      
      IF(IALF-NBP1)10,20,20
 10   CONTINUE
      RETURN
 20   IF(IALF-NBP2)30,30,10
 30   CONTINUE
      
      IF(ITEMP)40,50,50
      
 40   CONTINUE
      RETURN
 50   IF(IALF-IBP1)60,60,70
 60   IF(ITEMP-ITMP1)100,100,40
 70   IF(IALF-IBP2)80,80,90
 80   IF(ITEMP-ITMP2)100,100,40
 90   IF(ITEMP-ITMP3)100,100,40
 100  CONTINUE
      
      IIALF=IALF
      IDT=ITEMP-IBAS
      
      CALL VCF6C(IALF, IDT, IVCF)
      IFLAG=0
      
      IF(IALF-IBP1)101,101,110
 101  IF(ITEMP-IEP1)140,140,105
 105  IFLAG=1
      GO TO 140
 110  IF(IALF-IBP2)120,120,130
 120  IF(ITEMP-IEP2)140,140,105
 130  IF(ITEMP-IEP3)140,140,105 
 140  CONTINUE
      
      JVCF=(IVCF/1000+5)/10
      PVCF=JVCF
      PVCF=PVCF/10000.
      IF(IVCF-100000000)150,160,160
 150  CONTINUE
      
      JVCF=(IVCF/100+5)/10
      CVCF=JVCF
      CVCF=CVCF/100000.
      GO TO 180
 160  CONTINUE
      CVCF=PVCF
 180  CONTINUE
      VCFP=PVCF
      VCFC=CVCF
      RETURN
      END
      

      







      
      
      SUBROUTINE DIV6C(INUM,IDENOM,IRES,ISCALE)
      
      
      IRES1=INUM/IDENOM
      IRES2=(INUM-IRES1*IDENOM)*ISCALE/IDENOM
      
      IRES=IRES1*ISCALE+IRES2
      RETURN
      END
      
      
      SUBROUTINE VCF6C(IALF,IDT,IVCF)
      
      ITERM1=IALF*IDT
      ITERM2=ITERM1/5*4
      
      CALL MPY6C(ITERM1,ITERM2,ITERM3)
      IX=-ITERM1-ITERM3
      
      ISUM1=100000000+IX
      CALL MPY6C(IX,IX,ISUM2)
      ISUM2=ISUM2/2
      CALL MPY6C(IX,ISUM2,ISUM3)
      ISUM3=ISUM3/3
      CALL MPY6C(IX,ISUM3,ISUM4)
      ISUM4=ISUM4/4
      CALL MPY6C(IX,ISUM4,ISUM5)
      ISUM5=ISUM5/5
      CALL MPY6C(IX,ISUM5,ISUM6)
      ISUM6=ISUM6/6
      
      IVCF=ISUM1+ISUM2+ISUM3+ISUM4+ISUM5+ISUM6
      RETURN
      END
      
      
      SUBROUTINE MPY6C(IX,IY,IZ)
      
      ISCALE=10000
      IU1=IX/ISCALE
      K1=ISCALE*IU1
      IV1=IX-K1
      IU2=IY/ISCALE
      K2=ISCALE*IU2
      IV2=IY-K2
      K3=IU1*IV2+IU2*IV1+IV1*IV2/ISCALE
      IZ=(K3+ISCALE/2)/ISCALE + IU1*IU2
      RETURN
      END
      

      
